...
class GenreSystem(RatingSystem):
    def __init__(self, n_epochs=12, lr=0.015, reg=0.05, max_train_samples=2500000):
        ...
        print("Inicjalizacja GenreSystem...")
        ...
        print(f"GenreSystem używa {len(raw_train_data)} próbek (średnia globalna: {self.global_mean:.3f})")
        ...
        print(f"Start SGD: Zmapowano {u_idx} użytkowników, {m_idx} filmów, {len(optimized_train_data)} próbek.")
        ...
        for epoch in range(n_epochs):
            ...
            print(f"Epoka {epoch+1}/{n_epochs} zakończona w {time.time() - ep_start:.2f}s")
            
        print(f"GenreSystem gotowy (czas inicjalizacji: {time.time() - start_time:.2f}s)")
